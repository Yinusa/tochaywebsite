...
import { PRODUCT_DESIGN_PROJECTS } from "@/lib/projects-data";
import Footer from "@/components/ui/Footer";
import { supabase } from "@/lib/supabase";

export default function ProductDesignExplorePage() {
  const [projectList, setProjectList] = useState<any[]>(PRODUCT_DESIGN_PROJECTS);
    ...
    >
      ...
      >
        ...
        <div className="w-full md:block hidden overflow-hidden select-none">
          ...
          >
            {projectList.map((project, idx) => (
              <div key={`product-pc-${idx}`} className="flex flex-col gap-3 shrink-0">
                {project.has_details ? (
                  <Link
        ...
        <div className="w-full md:hidden flex flex-col gap-8 px-6">
          {projectList.map((project, idx) => (
            <div key={`product-mb-${idx}`} className="flex flex-col gap-3 w-full">
              {project.has_details ? (
                <Link
                  href={`/projects/${project.slug}`}
